...
public class Liste<T> {
    ...
    public void anfuegen(final T daten) {
        // Neuen Link anlegen
        Link<T> neu = new Link<>(daten, null);
    ...
    public int entferneWerte(final T opfer) {
        ...
        // Diese Methode wird im Praktikum implementiert
        // TODO
        // Wenn die Liste leer oder position < 0 ist, wird
        if (istLeer())
        {
            return 0;
        }
        Link<T> curent = anfang;
        Link<T> prev = null;
        while (curent != null)
        {
            if (curent.daten == opfer)
            {
                if (prev == null)
                {
                    anfang = curent.naechster;
                    anzGeloeschte++;
                } else
                {
                    prev.naechster = curent.naechster;
                    anzGeloeschte++;
                }
